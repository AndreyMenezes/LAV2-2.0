...
public class UC2 extends GraphicUC {
	...
	public UC2(PApplet p, String[] data1, String[] data2, String[] data3, String[] data4) {
		super(p);
		this.parent = p;
		this.data1 = data1;
		this.data2 = data2;
	...
	private String[][] loadCSV(String[] lines){
	...
	private void loadData() {
		...
		//"correlacoes.csv");
		//String[][] lines = loadCSV(parent.loadStrings("correlacoes.csv"));
		String[][] lines = loadCSV(data1);
		...
		//lines = loadCSV(parent.loadStrings("UC2-Prova1.csv"));
		lines = loadCSV(data2);
		for (int i = 1; i < lines.length; i++) {
		...
		//lines = loadCSV(parent.loadStrings("UC2-Prova2.csv"));
		lines = loadCSV(data3);
		for (int i = 1; i < lines.length; i++) {
			int length = lines[i].length;
			float[] tmp = new float[length];
		...
		//lines = loadCSV(parent.loadStrings("UC2-Prova3.csv"));
		lines = loadCSV(data4);
		for (int i = 1; i < lines.length; i++) {
			int length = lines[i].length;
			float[] tmp = new float[length];
